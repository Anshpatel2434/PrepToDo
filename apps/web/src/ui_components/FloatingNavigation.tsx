...
export const FloatingNavigation: React.FC<FloatingNavigationProps> = () => {
	...
	useEffect(() => {
		console.log(
			"88888888888888888888888888888 is logged out status ; ",
			isLogoutSuccess
		);
		const tempState = authState;
		setUser(tempState);
		setIsAuthenticated(tempState?.role === "authenticated");
		console.log(
			"||||||||||||||||||||||||||||Auth state in siderbar:",
			tempState
		);
		console.log(
			"||||||||||||||||||||||||||||Checking comparision in siderbar : ",
			tempState?.role === "authenticated"
		);
	}, [authState, isLogoutSuccess]);

	async function handleLogout() {
		try {
			console.log("status of logout loading ; ", isLogoutLoading);
			const data = await logout().unwrap();
			console.log("status of logout loading afterwarsd : ", isLogoutLoading);
			if (data.success) {
				console.log("|||||||||||||||logout success ? ", isLogoutSuccess);
				if (isLogoutSuccess) toast.success("Logged out successfully!");
				setIsOpen(!isOpen);
			}
		} catch (error) {
	...
	const onNavigate = (path: string, item: string) => {
		navigate(path);
	};
