...
const dailyPracticeSlice = createSlice({
    ...
    reducers: {
        ...
        initializeSession: (state, action: PayloadAction<{
            ...
            const { session, questionIds, existingAttempts, elapsedTime, status } = action.payload;
        ...
        submitAnswer: (state, action: PayloadAction<{
            ...
            const { questionId, userId, passageId, answer, isCorrect } = action.payload;
        ...
        toggleMarkForReview: (state, action: PayloadAction<{ questionId: UUID, userId: UUID, passageId: UUID | null }>) => {
            const { questionId, userId, passageId } = action.payload;
            const existing = state.attempts[questionId];

            if (existing) {
                state.attempts[questionId] = { ...existing, marked_for_review: !existing.marked_for_review };
            } else {
        ...
        updateConfidenceLevel : (state, action: PayloadAction<{questionId: UUID ,userId: UUID, passageId: UUID | null, confidence_level : number}>) => {
            const {questionId,confidence_level,userId,passageId} = action.payload
            const existing = state.attempts[questionId]

            if(existing){
                state.attempts[questionId] = {...existing, confidence_level: confidence_level}
            }
            else{
        ...
        clearResponse: (state) => {
            ...
            if (state.attempts[questionId]) {
                // We keep metadata like time spent and review tag, but nullify the answer
                state.attempts[questionId] = {
                    ...state.attempts[questionId],
...
export const selectDailyState = (state: { dailyPractice: DailyPracticeState }) => state.dailyPractice;
export const selectViewMode = createSelector(selectDailyState, s => s.viewMode);
export const selectSolutionViewType = (state: { dailyPractice: DailyPracticeState }) => 
    state.dailyPractice.solutionViewType;
export const selectCurrentQuestionIndex = createSelector(selectDailyState, s => s.currentQuestionIndex);
export const selectAttempts = createSelector(selectDailyState, s => s.attempts);
export const selectQuestionOrder = createSelector(selectDailyState, s => s.questionOrder);
export const selectElapsedTime = createSelector(selectDailyState, s => s.elapsedTimeSeconds);
export const selectShowPalette = createSelector(selectDailyState, s => s.showQuestionPalette);
...
export const selectProgressStats = createSelector(
    ...
    (attempts, order) => {
        ...
        const answered = Object.values(attempts).filter(a => a.user_answer?.user_answer != null).length;
        const correct = Object.values(attempts).filter(a => a.is_correct).length;
        return { total, answered, correct, percentage: total > 0 ? Math.round((answered/total)*100) : 0 };
    }
);
