...
export const QuestionPalette: React.FC<QuestionPaletteProps> = ({
    ...
    const getStatus = (questionId: string) => {
        ...
        if (pending) {
            const hasAnswer = (pending.user_answer as Record<string, unknown>)?.user_answer != null;
        ...
        const hasAnswer = (attempt.user_answer as Record<string, unknown>)?.user_answer != null;
        return hasAnswer ? "answered" : "not_visited";
    };
    ...
    const stats = React.useMemo(
        () => ({
            answered: Object.values(allAttempts).filter(
                (a) =>
                    (a.user_answer as Record<string, unknown>)?.user_answer != null && !a.marked_for_review
            ).length,
            marked: Object.values(allAttempts).filter((a) => a.marked_for_review)
                .length,
    ...
    return (
        ...
        >
                ...
                <div className="grid grid-cols-5 gap-2">
                    {questions.map((q, i) => (
                        <motion.button
                            ...
                            className={`
                                aspect-square flex items-center justify-center rounded-lg font-medium text-sm border-2 ${isDark
            ...
            {/* Footer */}
            <div
                className={`p-4 border-t ${isDark ? "border-border-dark" : "border-border-light"
                    }`}
            >
