...
export const QuestionPanel: React.FC<QuestionPanelProps> = ({
    ...
    }, [question.id, currentAttempt?.id]);

    const handleGenerateInsight = async () => {
        if (!currentAttempt?.id || !session.id || !question.id) return;
        setInsightError(null);
        try {
            ...
            setInsightError(err.message || 'Something went wrong');
        }
    };
    ...
    const renderAIInsights = () => {
        ...
        // Check for existing diagnostic (from session analytics or local state)
        const existingDiagnostic = localDiagnostic || session.analytics?.diagnostics?.find(
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const renderDiagnosticContent = (diagnostic: any) => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const focusAreas = diagnostic.focus_areas || (diagnostic.related_weak_areas?.map((a: any) => a.human_readable_description) || []);
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                {getSentences(question).map((s) => (
                                    <div
                                        key={s.id}
                                        className={`py-2 flex gap-3 ${isDark
                            ...
                            <div className="leading-relaxed opacity-90 whitespace-pre-line">
                                {
                                    solutionViewType === "personalized"
                                        ? renderAIInsights()
                                        : question.rationale.split(/(".*?"|'.*?')/g).map((part, i) =>
                                            (part.startsWith('"') && part.endsWith('"')) || (part.startsWith("'") && part.endsWith("'")) ? (
                                                <span key={i} className={` font-semibold ${isDark ? "text-text-tertiary-dark" : "text-text-tertiary-light"}`}>
                                                    {part}
                                                </span>
                                            ) : (
                                                part
