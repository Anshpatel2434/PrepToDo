...
import { selectConfidenceLevel, setConfidenceLevel } from '../../redux_usecase/dailyPracticeSlice';
...
export const ConfidenceSelector: React.FC<ConfidenceSelectorProps> = ({ isDark, disabled = false }) => {
    const dispatch = useDispatch();
    const confidenceLevel = useSelector(selectConfidenceLevel);

    const options = [
        { level: 1, label: 'Low', icon: MdOutlineSentimentDissatisfied, color: 'error' },
        { level: 2, label: 'Med', icon: MdOutlineSentimentNeutral, color: 'warning' },
        { level: 3, label: 'High', icon: MdOutlineSentimentSatisfied, color: 'success' },
    ];
    ...
    const getOptionStyles = (level: number) => {
        ...
        return `${baseStyles} ${isDark ? 'bg-bg-tertiary-dark border-border-dark text-text-muted-dark' : 'bg-bg-tertiary-light border-border-light text-text-muted-light'} hover:border-brand-primary-${isDark ? 'dark' : 'light'}`;
    };
            ...
            <div className="flex gap-2">
                {options.map((option) => {
                    ...
                    return (
                        <motion.button
                            key={option.level}
                            onClick={() => !disabled && dispatch(setConfidenceLevel(option.level))}
