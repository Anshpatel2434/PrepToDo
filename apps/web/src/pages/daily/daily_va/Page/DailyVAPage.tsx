                    ...
                    <div className="space-y-3">
                        ...
                        <input
                            ...
                            className={`
                                ...
                                ${!isExamMode && currentQuestion.correctAnswer === selectedOption ? 'border-success' : ''}
                                ${!isExamMode && currentQuestion.correctAnswer !== selectedOption ? 'border-error' : ''}
                            `}
                        />
                        <p className={`
                            text-xs
            ...
            <div className="space-y-3">
                {currentQuestion.options.map((option, index) => {
                    ...
                    if (isExamMode) {
                        ...
                        if (isCorrect) {
                            optionClass += isDark ? 'bg-success/20 border-success text-success' : 'bg-success/10 border-success text-success';
                        } else if (isSelected && !isCorrect) {
                            optionClass += isDark ? 'bg-error/20 border-error text-error' : 'bg-error/10 border-error text-error';
                        } else {
                            optionClass += isDark ? 'bg-bg-tertiary-dark border-border-dark text-text-muted-dark' : 'bg-bg-tertiary-light border-border-light text-text-muted-light';
                        }
                    }
                        ...
                        >
                            <div className="flex items-start gap-3">
                                <span className={`
                                    ...
                                    ${!isExamMode && currentQuestion.correctAnswer === option.id 
                                        ? isDark ? 'bg-success text-white' : 'bg-success text-white'
                                        : ''
                                    }
                                    ${!isExamMode && selectedOption === option.id && currentQuestion.correctAnswer !== option.id
                                        ...
                                        {[1, 2, 3].map((level) => (
                                            <motion.button
                                                ...
                                                className={`
                                                    ...
                                                    ${selectedOption === String(level)
                                                        ? level === 1 
                                                            ? isDark ? 'bg-error/20 border-error text-error' : 'bg-error/10 border-error text-error'
                                                            : level === 2
                                                                ? isDark ? 'bg-warning/20 border-warning text-warning' : 'bg-warning/10 border-warning text-warning'
                            ...
                            <div className="px-6 py-4">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <div className="flex gap-4 text-xs">
                                        <span className="flex items-center gap-1">
                                            ...
                                            Answered ({Object.values(attempts).filter(a => a.status === 'answered').length})
                                        </span>
                                        <span className="flex items-center gap-1">
                                            <span className="w-3 h-3 rounded bg-error" />
                                            Skipped ({Object.values(attempts).filter(a => a.status === 'skipped').length})
                                        </span>
                                        <span className="flex items-center gap-1">
                                            <span className="w-3 h-3 rounded bg-info" />
                                            Marked ({Object.values(attempts).filter(a => a.status === 'marked_for_review').length})
                                        </span>
                                    </div>
                                </div>
                                <div className="flex gap-2 justify-center">
                                    {questions.map((q, index) => {
                                        ...
                                        let bgClass = isDark ? 'bg-bg-tertiary-dark' : 'bg-bg-tertiary-light';
                                        if (status === 'answered') bgClass = isDark ? 'bg-success/80' : 'bg-success';
                                        else if (status === 'skipped') bgClass = isDark ? 'bg-error/80' : 'bg-error';
                                        else if (status === 'marked_for_review') bgClass = isDark ? 'bg-info/80' : 'bg-info';
