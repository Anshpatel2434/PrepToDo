...
/**
 * Generates rationales for RC questions using reasoning graph edges.
 */
export async function generateRationalesWithEdges(params: {
    ...
    console.log(`🧾 [RC Rationales] Generating rationales for ${questions.length} questions`);
...
${context.edges.map(e => `- ${e.source_node_label} → ${e.relationship} → ${e.target_node_label}`).join("\n")}`;
    ...
    const prompt = `You are a CAT VARC examiner. Write elimination-driven rationales for questions.
...
3. Uses the reasoning context (metrics, nodes, edges) to frame the explanation
4. Does NOT reveal the internal structure (no "PART 1", no node labels)
...
- Keep rationales concise (2-3 sentences per option)
`;

    console.log("⏳ [RC Rationales] Waiting for LLM to generate rationales");
    ...
    const completion = await client.chat.completions.parse({
        ...
        response_format: zodResponseFormat(RationaleResponseSchema, "rc_rationales"),
    });

    const parsed = completion.choices[0].message.parsed;

    if (!parsed) {
        throw new Error("Failed to generate RC rationales");
    }
    ...
    console.log(`✅ [RC Rationales] Generated rationales for all questions`);
