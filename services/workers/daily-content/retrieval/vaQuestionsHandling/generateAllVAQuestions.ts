...
CRITICAL MINDSET:
- CAT VA questions are NOT language exercises. They are logic tests.
...
**Anti-Patterns**: DO NOT let correct option be longest. DO NOT use moral judgement words unless author does. DO NOT repeat passage vocabulary excessively.
...
**Anti-Patterns**: DO NOT use grammar errors to eliminate options. DO NOT make only one blank "sound right". DO NOT allow multiple plausible placements.
...
- "However" implies a preceding thought; user cannot find the start if the start is "However".

**Generation Process**:
1. Identify Opening sentence (contextual, general) - **Check: Does NOT start with transition**.
2. Identify Closing sentence (insight, implication).
3. Ensure every sentence depends on at least one other.
4. Create traps using thematic similarity and chronological illusion.
