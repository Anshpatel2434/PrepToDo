...
`;
    ...
    const completion = await client.chat.completions.parse({
        ...
        messages: [
            {
                role: "system",
                content: "You are a CAT VARC examiner. You design reasoning questions with carefully constructed traps. You also select the correct answer for each question based on the passage.",
            },
            {
                role: "user",
                content: prompt,
    ...
    const distribution = shuffledQuestions.reduce((acc: Record<string, number>, q: any) => {
