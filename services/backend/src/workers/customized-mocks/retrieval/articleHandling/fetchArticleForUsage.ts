...
export async function fetchArticleForUsage(params: {
    ...
    user_id?: string; // Optional user_id if needed for future logic, but we ignore it for now as per daily-content logic
}) {
    ...
    let semanticData = { semantic_ideas: {}, authorial_persona: {} };
    if (selectedArticle.semantic_ideas_and_persona) {
        if (typeof selectedArticle.semantic_ideas_and_persona === 'string') {
            try {
