...
// Generated: 2026-01-27T10:37:13.414Z
// Function: daily-content-passage
...
// workers/daily-content/retrieval/utils/CostTracker.ts
var CostTracker = class {
  constructor() {
    __publicField(this, "calls", []);
    // OpenAI gpt-4o-mini pricing (per 1M tokens)
    __publicField(this, "INPUT_COST_PER_MILLION", 0.15);
    __publicField(this, "OUTPUT_COST_PER_MILLION", 0.6);
  }
  /**
   * Log an API call with token usage
   */
  logCall(functionName, inputTokens, outputTokens) {
    const call = {
      functionName,
      inputTokens,
      outputTokens,
    ...
    console.log(`\u{1F4B0} [Cost] ${functionName}: ${inputTokens} in, ${outputTokens} out (~$${callCost.toFixed(4)})`);
    if (inputTokens > 1e4) {
   ...
   */
  calculateCallCost(inputTokens, outputTokens) {
    const inputCost = inputTokens / 1e6 * this.INPUT_COST_PER_MILLION;
    const outputCost = outputTokens / 1e6 * this.OUTPUT_COST_PER_MILLION;
  ...
  printReport() {
    ...
    console.log("\n" + "=".repeat(70));
    console.log("\u{1F4B0} COST TRACKER REPORT");
    ...
    for (const item of report.breakdown) {
      console.log(
        `${item.function.padEnd(35)} | In: ${String(item.inputTokens).padStart(6)} | Out: ${String(item.outputTokens).padStart(5)} | $${item.cost.toFixed(4)} (${item.percentage.toFixed(1)}%)`
      );
    }
    console.log("=".repeat(70) + "\n");
  }
...
// workers/daily-content/retrieval/passageHandling/generatePassage.ts
var client = new OpenAI();
var MODEL = "gpt-4o-mini";
async function generatePassage(params, costTracker) {
...
// workers/daily-content/functions/step2Passage/handler.ts
async function handleStep2Passage(params) {
  const { exam_id } = params;
  console.log(`\u{1F680} [Step 2] Passage generation: ${exam_id}`);
  try {
    return await ErrorHandler.withErrorHandling(exam_id, "Step 2: Passage", async () => {
      console.log(`\u{1F4D6} [Step 2] Loading state`);
      ...
      const passageId = v4_default();
      console.log(`\u{1F4BE} [Step 2] Saving passage: ${passageId}`);
      ...
      console.log(`\u{1F4DE} [Step 2] Invoking Step 3: RC Questions`);
      await FunctionInvoker.invokeNext("step-3", { exam_id });
      console.log(`\u{1F389} [Step 2] Passage generation complete!`);
