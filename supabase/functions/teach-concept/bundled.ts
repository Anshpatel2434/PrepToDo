...
// Generated: 2025-12-27T07:16:09.500Z
...
// ai-orchestration/teaching-concept/config/openai.ts
var openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY
});

// ai-orchestration/teaching-concept/retrieval/generateEmbedding.ts
...
// ai-orchestration/teaching-concept/config/supabase.ts
var supabase = createClient(
  process.env.SUPABASE_URL,
  process.env.SUPABASE_SERVICE_ROLE_KEY
);

// ai-orchestration/teaching-concept/retrieval/searchTheoryEmbeddings.ts
async function searchTheoryEmbeddings(queryEmbedding, topK = 5) {
  console.log("\u{1F4D0} [Vector Search] Querying theory embeddings");
...
// ai-orchestration/teaching-concept/retrieval/fetchTheoryChunk.ts
async function fetchTheoryChunk(theoryId) {
  console.log("\u{1F4D8} [Theory] Fetching theory chunk:", theoryId);
  const { data, error } = await supabase.from("theory_chunks").select("*").eq("id", theoryId).single();
  if (error)
    throw error;
...
// ai-orchestration/teaching-concept/graph/expandConceptGraph.ts
async function expandConceptGraph(conceptNodeId) {
  console.log("\u{1F9E0} [Graph Traversal] Expanding graph for node:", conceptNodeId);
  const { data: edges, error } = await supabase.from("graph_edges").select(
    `
            relationship,
            source_node_id
            `
  ...
  console.log(
    `\u{1F517} [Graph Traversal] Found ${edges.length} outgoing edges and they are  : `
  );
  console.log(edges);
  if (edges.length === 0)
    return [];
  const targetNodeIds = edges.map((e) => e.source_node_id);
  const { data: nodes, error: nodeError } = await supabase.from("graph_nodes").select("id, label, type").in("id", targetNodeIds);
  if (nodeError) {
    console.error("\u274C [Graph Traversal] Node fetch failed:", nodeError);
    throw nodeError;
  }
  console.log(
    `\u{1F4DA} [Graph Traversal] Loaded ${nodes.length} target nodes and they are : `
  );
  console.log(nodes);
...
// ai-orchestration/teaching-concept/assembly/buildTeachingContext.ts
function buildTeachingContext(theoryChunk, graphRelations) {
  console.log("\u{1F9E9} [Assembly] Building teaching context");
  const context = {
...
// ai-orchestration/teaching-concept/synthesis/narrateConcept.ts
var client = new OpenAI();
async function narrateConcept(teachingContext) {
  console.log("\u{1F5E3}\uFE0F [Narration] Synthesizing explanation");
...
async function runConceptTeaching(conceptQuery) {
  console.log("\u{1F680} [START] Concept Teaching:", conceptQuery);
  ...
  console.log(
    "\u{1F3AF} [Selection] Using theory_id:",
  ...
  console.log("\u{1F3C1} [END] Concept Teaching complete");
  return explanation;
}
export {
  runConceptTeaching
