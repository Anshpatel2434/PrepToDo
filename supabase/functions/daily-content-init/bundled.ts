...
// Generated: 2026-01-26T19:08:11.492Z
// Function: daily-content-init
...
// workers/daily-content/shared/stateManager.ts
var StateManager = class {
  /**
   * Load generation state for daily content
   */
  static async load(examId) {
    console.log(`\u{1F4D6} [StateManager] Loading state for daily content: ${examId}`);
  ...
  static async update(examId, updates) {
    console.log(`\u{1F4BE} [StateManager] Updating state for daily content: ${examId}`);
  ...
  static async markCompleted(examId) {
    console.log(`\u{1F389} [StateManager] Marking daily content as completed: ${examId}`);
   ...
   */
  static async create(examId, genre, totalSteps = 8) {
    console.log(`\u{1F4DD} [StateManager] Creating initial state for daily content: ${examId}`);
    const { error } = await supabase.from("exam_generation_state").insert({
      exam_id: examId,
      status: "initializing",
      current_step: 1,
...
// workers/daily-content/shared/functionInvoker.ts
...
// workers/daily-content/shared/errorHandler.ts
...
// workers/daily-content/retrieval/fetchGenre.ts
async function fetchGenreForToday() {
  console.log("\u{1F680} [GENRE] Fetching genre start ");
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const cooldownDate = new Date(Date.now() - 24 * 60 * 60 * 1e3).toISOString();
  const { data: genres, error } = await supabase.from("genres").select("*").eq("is_active", true).or(`last_used_daily_at.is.null,last_used_daily_at.lt.${cooldownDate}`).order("daily_usage_count", { ascending: true }).order("last_used_daily_at", { ascending: true, nullsFirst: true }).limit(1);
  if (error) {
  ...
  console.log("\u{1F680} [GENRE] Genre selected :  ", selectedGenre.name);
  const { error: updateError } = await supabase.from("genres").update({
...
// workers/daily-content/retrieval/articleHandling/fetchArticleForUsage.ts
async function fetchArticleForUsage(params) {
  const { genre, usageType } = params;
  console.log(
    `\u{1F680} [ARTICLE] Fetching article | genre=${genre}, usage=${usageType}`
  );
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
// workers/daily-content/retrieval/generateEmbedding.ts
...
// workers/daily-content/retrieval/searchPassageAndQuestionEmbeddings.ts
...
// workers/daily-content/retrieval/passageHandling/fetchPassagesData.ts
...
// workers/daily-content/retrieval/fetchQuestionsData.ts
...
// workers/daily-content/functions/step1Init/handler.ts
async function handleStep1Init() {
  const examId = v4_default();
  console.log(`\u{1F680} [Step 1] Daily content initialization: ${examId}`);
  try {
    return await ErrorHandler.withErrorHandling(examId, "Step 1: Init", async () => {
      console.log(`\u{1F3AF} [Step 1.1] Selecting genre for today`);
      const genre = await fetchGenreForToday();
      console.log(`   Selected genre: ${genre.name}`);
      console.log(`\u{1F4C4} [Step 1.2] Fetching article for genre: ${genre.name}`);
      const { articleMeta, semantic_ideas, authorial_persona } = await fetchArticleForUsage({
        genre: genre.name,
        usageType: "daily"
      });
      console.log(`   Article: ${articleMeta.title}`);
      console.log(`\u{1F9E0} [Step 1.3] Generating embedding and fetching PYQ references`);
      ...
      console.log(`\u{1F4DD} [Step 1.4] Creating daily content exam record`);
      ...
      console.log(`\u{1F4BE} [Step 1.5] Creating generation state`);
      await StateManager.create(examId, genre.name, 8);
      console.log(`\u{1F5C2}\uFE0F [Step 1.6] Saving initial data to state`);
      ...
      console.log(`\u{1F4DE} [Step 1.7] Invoking Step 2: Passage generation`);
