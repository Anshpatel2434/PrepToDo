// Auto-generated bundle for Deno Edge Runtime
// Generated: 2026-01-30T12:20:29.189Z
// Function: customized-mocks-init
...
// workers/customized-mocks/retrieval/generateEmbedding.ts
async function generateEmbedding(text) {
  console.log("\u{1F9E0} [Embedding] Generating query embedding");
...
var init_generateEmbedding = __esm({
  "workers/customized-mocks/retrieval/generateEmbedding.ts"() {
    init_openai2();
  }
});

// workers/customized-mocks/retrieval/searchPassageAndQuestionEmbeddings.ts
async function searchPassageAndQuestionEmbeddings(queryEmbedding, topK = 5) {
  console.log(`\u{1F50E} [Vector Search] Searching similar passages/questions (topK=${topK})`);
  ...
  console.log(`\u{1F50E} [Vector Search] Searching similar questions by type (perType=${topK})`);
...
var init_searchPassageAndQuestionEmbeddings = __esm({
  "workers/customized-mocks/retrieval/searchPassageAndQuestionEmbeddings.ts"() {
    init_supabase();
  }
});

// workers/customized-mocks/retrieval/passageHandling/fetchPassagesData.ts
async function fetchPassagesData(passageIds) {
  console.log(`\u{1F4C4} [Passages] Fetching ${passageIds.length} passages from DB`);
  const { data, error } = await supabase.from("passages").select("*").in("id", passageIds);
  if (error) {
...
var init_fetchPassagesData = __esm({
  "workers/customized-mocks/retrieval/passageHandling/fetchPassagesData.ts"() {
    init_supabase();
  }
});

// workers/customized-mocks/retrieval/fetchQuestionsData.ts
async function fetchQuestionsData(questionsIds, passageIds) {
  console.log(
    `\u2753 [Questions] Fetching questions from DB (questionIds=${questionsIds.length}, passageIds=${passageIds.length})`
  );
  const { data, error } = await supabase.from("questions").select("*").or(`id.in.(${questionsIds.join(",")}),passage_id.in.(${passageIds.join(",")})`);
  if (error) {
...
var init_fetchQuestionsData = __esm({
  "workers/customized-mocks/retrieval/fetchQuestionsData.ts"() {
    init_supabase();
  }
});

// workers/customized-mocks/shared/referenceDataHelpers.ts
var referenceDataHelpers_exports = {};
__export(referenceDataHelpers_exports, {
  fetchPassagesData: () => fetchPassagesData,
  fetchQuestionsData: () => fetchQuestionsData,
  generateEmbedding: () => generateEmbedding,
  searchPassageAndQuestionEmbeddings: () => searchPassageAndQuestionEmbeddings
});
var init_referenceDataHelpers = __esm({
  "workers/customized-mocks/shared/referenceDataHelpers.ts"() {
    init_generateEmbedding();
    init_searchPassageAndQuestionEmbeddings();
    init_fetchPassagesData();
    init_fetchQuestionsData();
  }
});
...
// workers/customized-mocks/functions/step1Init/handler.ts
init_supabase();

// workers/customized-mocks/shared/stateManager.ts
init_supabase();
var StateManager = class {
  /**
   * Load generation state for an exam
   */
  static async load(examId) {
    console.log(`\u{1F4D6} [StateManager] Loading state for exam: ${examId}`);
    const { data, error } = await supabase.from("exam_generation_state").select("*").eq("exam_id", examId).single();
    if (error) {
  ...
  static async update(examId, updates) {
    console.log(`\u{1F4BE} [StateManager] Updating state for exam: ${examId}`);
    const { error } = await supabase.from("exam_generation_state").update(updates).eq("exam_id", examId);
    if (error) {
   ...
   */
  static async markFailed(examId, errorMessage) {
    console.error(`\u274C [StateManager] Marking exam as failed: ${examId}`);
    console.error(`   Error: ${errorMessage}`);
    await Promise.all([
      supabase.from("exam_generation_state").update({
        status: "failed",
        error_message: errorMessage
      }).eq("exam_id", examId),
      supabase.from("exam_papers").update({ generation_status: "failed" }).eq("id", examId)
    ]);
  }
  /**
   * Mark as completed and cleanup
   */
  static async markCompleted(examId) {
    console.log(`\u{1F389} [StateManager] Marking exam as completed: ${examId}`);
    await supabase.from("exam_papers").update({ generation_status: "completed" }).eq("id", examId);
    await supabase.from("exam_generation_state").delete().eq("exam_id", examId);
    console.log(`\u2705 [StateManager] Exam marked as completed and state cleaned up`);
  }
};

// workers/customized-mocks/shared/functionInvoker.ts
init_supabase();
init_dist4();
var FunctionInvoker = class {
  static async invokeNext(step, payload) {
    const functionName = this.FUNCTION_MAP[step];
    if (!functionName) {
      throw new Error(`Unknown step: ${step}`);
    }
    console.log(`\u{1F4DE} [FunctionInvoker] Invoking ${functionName}`);
    try {
      ...
      console.log(`\u{1F680} [FunctionInvoker] Triggered ${functionName}, not waiting for response`);
    } catch (err) {
...
__publicField(FunctionInvoker, "FUNCTION_MAP", {
...
// workers/customized-mocks/shared/errorHandler.ts
var ErrorHandler = class {
  /**
   * Handle error in a function step
   */
  static async handle(examId, step, error) {
   ...
   */
  static async withErrorHandling(examId, step, fn) {
    try {
      return await fn();
    } catch (error) {
...
// workers/customized-mocks/retrieval/articles/fetchArticleForUsage.ts
init_supabase();
async function fetchArticleForUsage(params) {
  const { genre, usageType, user_id } = params;
  console.log(
    `\u{1F680} [ARTICLE] Fetching article | genre=${genre}, usage=${usageType}, user_id=${user_id || "N/A"}`
  );
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const cooldownDate = new Date(
    Date.now() - 100 * 24 * 60 * 60 * 1e3
  ).toISOString();
  let userArticleIds = [];
  if (usageType === "mock" && user_id) {
    console.log(`\u{1F4CA} [ARTICLE] Fetching user's exam history for user_id=${user_id}`);
    const { data: userExams, error: examsError } = await supabase.from("exam_papers").select("id").eq("generated_by_user_id", user_id);
    if (examsError) {
      ...
      console.log(`\u{1F4CB} [ARTICLE] Found ${userExamIds.length} exams for this user`);
      const { data: userPassages, error: passagesError } = await supabase.from("passages").select("article_id").in("paper_id", userExamIds);
      if (passagesError) {
        ...
        console.log(`\u{1F6AB} [ARTICLE] Excluding ${userArticleIds.length} articles already used by this user`);
      }
    } else {
      console.log(`\u2728 [ARTICLE] New user - no previous exams found`);
    }
  }
  let query = supabase.from("articles").select("*").eq("genre", genre).eq("is_archived", false);
  ...
  console.log(`\u{1F4DA} [ARTICLE] Total articles in genre: ${allArticles.length}`);
  ...
  if (usageType === "daily") {
    eligibleArticles = allArticles.filter((article) => {
      ...
      const customExamReusable = article.used_in_custom_exam && article.last_used_at && new Date(article.last_used_at) < new Date(cooldownDate);
      return neverUsed || customExamReusable;
    });
  } else {
    const usedByOthersArticles = allArticles.filter(
      (article) => article.used_in_custom_exam && !userArticleIds.includes(article.id)
    );
    const neverUsedArticles = allArticles.filter(
      (article) => !article.used_in_daily && !article.used_in_custom_exam && !userArticleIds.includes(article.id)
    );
    const dailyReusableArticles = allArticles.filter(
      (article) => article.used_in_daily && article.last_used_at && new Date(article.last_used_at) < new Date(cooldownDate) && !userArticleIds.includes(article.id)
    );
    if (usedByOthersArticles.length > 0) {
      eligibleArticles = usedByOthersArticles;
      console.log(`\u2705 [ARTICLE] Priority 1: ${usedByOthersArticles.length} articles used by others`);
    } else if (neverUsedArticles.length > 0) {
      eligibleArticles = neverUsedArticles;
      console.log(`\u2705 [ARTICLE] Priority 2: ${neverUsedArticles.length} never-used articles`);
    } else if (dailyReusableArticles.length > 0) {
      eligibleArticles = dailyReusableArticles;
      console.log(`\u2705 [ARTICLE] Priority 3: ${dailyReusableArticles.length} daily reusable articles`);
    } else {
      console.warn(`\u26A0\uFE0F [ARTICLE] All articles exhausted, ignoring user exclusions`);
      eligibleArticles = allArticles.filter((article) => {
        const neverUsed = !article.used_in_daily && !article.used_in_custom_exam;
        const dailyReusable = article.used_in_daily && article.last_used_at && new Date(article.last_used_at) < new Date(cooldownDate);
  ...
  eligibleArticles.sort((a, b) => {
    ...
    if (aUsed !== bUsed)
      return aUsed ? 1 : -1;
    const aCount = usageType === "daily" ? a.custom_exam_usage_count ?? 0 : a.daily_usage_count ?? 0;
    const bCount = usageType === "daily" ? b.custom_exam_usage_count ?? 0 : b.daily_usage_count ?? 0;
    if (aCount !== bCount)
  ...
  console.log(
    "\u{1F4D8} [ARTICLE] Article selected:",
    selectedArticle.title,
    "|",
    selectedArticle.url
  );
  const updatePayload = usageType === "daily" ? {
    used_in_daily: true,
  ...
  const { error: updateError } = await supabase.from("articles").update(updatePayload).eq("id", selectedArticle.id);
  if (updateError) {
...
// workers/customized-mocks/retrieval/updateGenre.ts
init_supabase();
async function updateGenres(genreNames) {
  const promises = genreNames.map(
    (name) => supabase.rpc("increment_genre_custom_usage", { genre_name: name })
  );
...
// workers/customized-mocks/functions/step1Init/handler.ts
async function handleStep1Init(params) {
  const examId = v4_default();
  console.log(`\u{1F680} [Step 1] Starting initialization for exam: ${examId}`);
  ...
  try {
    return await ErrorHandler.withErrorHandling(examId, "Step 1: Init", async () => {
      ...
      const { count, error: countError } = await supabase.from("exam_papers").select("*", { count: "exact", head: true }).eq("generated_by_user_id", user_id).neq("name", "Daily Practice").in("generation_status", ["initializing", "generating", "completed"]);
      if (countError) {
        console.error(`\u274C [Step 1] Failed to check mock count:`, countError);
        throw new Error(`Failed to check mock count: ${countError.message}`);
      }
      console.log(`\u{1F4CA} [Step 1] User ${user_id} has ${count} existing customized mocks`);
      ...
      console.log(`\u{1F4DD} [Step 1.1] Creating exam record`);
      const { data: examData, error: examError } = await supabase.from("exam_papers").insert({
        id: examId,
        name: mock_name,
      ...
      console.log(`\u{1F4DD} [Step 1.2] Creating generation state`);
      const { error: stateError } = await supabase.from("exam_generation_state").insert({
        exam_id: examId,
        status: "initializing",
        current_step: 1,
        total_steps: 7,
      ...
      console.log(`
\u{1F3AF} [Step 1.2.1] Genres: ${target_genres.join(", ")}`);
      await updateGenres(target_genres);
      console.log(`\u{1F4DA} [Step 1.3] Fetching ${num_passages} articles`);
      const articlesData = [];
      ...
      console.log(`\u{1F4DD} [Step 1.3.1] Updating the used articles ids in exam`);
      const articleIds = articlesData.map((a) => a.articleMeta.id);
      const { error: articleIdsError } = await supabase.from("exam_papers").update({ used_articles_id: articleIds }).eq("id", examId);
      if (articleIdsError) {
        throw new Error(`Failed to update article IDs in exam: ${articleIdsError.message}`);
      }
      console.log(`\u{1F9E0} [Step 1.4] Preparing reference data for question generation`);
      const { generateEmbedding: generateEmbedding2, searchPassageAndQuestionEmbeddings: searchPassageAndQuestionEmbeddings2, fetchPassagesData: fetchPassagesData2, fetchQuestionsData: fetchQuestionsData2 } = await Promise.resolve().then(() => (init_referenceDataHelpers(), referenceDataHelpers_exports));
      const embedding = await generateEmbedding2(target_genres.join(" "));
      const matches = await searchPassageAndQuestionEmbeddings2(embedding, 3);
      const passages = await fetchPassagesData2(matches.passages.map((m) => m.passage_id));
      const questions = await fetchQuestionsData2(
      ...
      const referenceDataVA = passages.slice(0, 2).map((p) => ({
        passage: p,
        questions: questions.filter((q) => q.passage_id === null || q.passage_id === void 0)
      }));
      console.log(`\u2705 [Step 1.4] Reference data prepared (${passagesContent.length} passages, ${questions.length} questions)`);
      console.log(`\u{1F4BE} [Step 1.5] Saving all data to state`);
      ...
      console.log(`\u{1F4DE} [Step 1.6] Invoking Step 2: Passages`);
      await FunctionInvoker.invokeNext("step-2", { exam_id: examId });
      console.log(`\u{1F389} [Step 1] Initialization complete!`);
      return {
        success: true,
        exam_id: examId,
        current_step: 1,
